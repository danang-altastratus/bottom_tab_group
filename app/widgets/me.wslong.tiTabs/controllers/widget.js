...
$.init = function(_params) {
  ...
  width = excess ? Math.floor(display.width / excessLength) : Math.floor((display.width-110) / (nodes.length-1));
  ...
  for (var i = 0; i < nodes.length; i++) {
    var tabController = Widget.createController("singleTab", {
      ...
      tabWidth : nodes[i].width||width
    });
    nodes[i].tabController = tabController;
    $.TabContainer1.add(tabController.getView());
  }
};
